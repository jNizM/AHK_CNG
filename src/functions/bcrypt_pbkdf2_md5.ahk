...
bcrypt_pbkdf2_md5(password, salt, iterations := 4096, keysize := 16, encoding := "utf-8")
{
	...
	try
	{
		...
		if (NT_STATUS := DllCall("bcrypt\BCryptOpenAlgorithmProvider", "ptr*", hAlg, "ptr", &BCRYPT_MD5_ALGORITHM, "ptr", 0, "uint", BCRYPT_ALG_HANDLE_HMAC_FLAG) != 0)
